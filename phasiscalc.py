...
def aureole_penalty_mag(D_deg, eps_deg, g=0.80, A0=1.5, D0=2.0):
    ...
    # HG phase function (정규화: 90°에서 1이 되도록)
...
def _swe_calc_ut_cached(jd_ut, ipl, flags):
    rflag, xx, serr = astrology.swe_calc_ut(float(jd_ut), int(ipl), int(flags))
    # rflag는 Morinus 래퍼가 튜플을 줄 수 있으므로 그대로 보존한다.
    return rflag, (float(xx[0]), float(xx[1]), float(xx[2])), ("" if serr is None else str(serr))
