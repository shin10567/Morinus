...
def _ra_dec_star_deg_ut(jd_ut, star_name):
    ...
    iflag = swe.FLG_SWIEPH | swe.FLG_TRUEPOS | swe.FLG_EQUATORIAL
...
def _ra_dec_planet_deg_ut(jd_ut, ipl, lon_deg, lat_deg, alt_m=0.0):
    ...
    swe.set_topo(lon_deg, lat_deg, alt_m)
    iflag = swe.FLG_SWIEPH | swe.FLG_EQUATORIAL | swe.FLG_TOPOCTR
    xx, _ = swe.calc_ut(jd_ut, ipl, iflag)
    return xx[0], xx[1]
...
_PLANET_LABEL = {
    swe.SUN: u"Sun", swe.MOON: u"Moon", swe.MERCURY: u"Mercury",
    swe.VENUS: u"Venus", swe.MARS: u"Mars", swe.JUPITER: u"Jupiter",
    swe.SATURN: u"Saturn"
}
...
class ParanatellontaWnd(cw.CommonWnd):
    ...
    def _compute_rows(self):
        ...
        jd_ut = getattr(self.horoscope, "jd_ut",
                        getattr(getattr(self.horoscope, "time", None), "jd", None))
        if jd_ut is None:
            return []
